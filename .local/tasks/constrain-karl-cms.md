...
- Transaction-specific sections: **What to know**, **What to do** (hard-coded headings, cannot be renamed; contains Callout, Section, Address, Document, Email, Button link, Phone number, Text)

**Wagtail style guide reference:** The file `attached_assets/Styleguide_-_Wagtail_1776266282756.html` is the Karl CMS admin interface style guide (from `api.sf.gov/admin/styleguide/`). It shows the exact field patterns and component UI that editors encounter in Karl. When naming components and describing their structure in prompts, match the field labels and patterns shown in this guide — for example, rich text fields use bold, h3/h4, bulleted/numbered lists, blockquote, and link; Section components contain a heading and a rich text body; Callout components contain a short text block. This ensures generated drafts describe content in terms editors can directly enter into the Karl admin UI.
...
- Component descriptions in the system prompt use field names and patterns consistent with what editors see in the Karl admin interface (e.g., rich text fields supporting bold, h3/h4, lists; Section with heading + body; Callout as a short text block with optional link).
...
- `attached_assets/Styleguide_-_Wagtail_1776266282756.html`
