...
const createFileStore = async (filePath = DEFAULT_LOCAL_DB_PATH) => {
  ...
  return {
    ...
    async getPage(id) {
      const row = state.pages.find(
        (entry) =>
          String(entry.id) === String(id) || String(entry.data?.id) === String(id)
      );
      if (!row) return null;
      const maxVersion = state.page_versions
        .filter((entry) => String(entry.page_id) === String(row.id))
        .reduce((max, entry) => {
      ...
      const data = clone(row.data);
      data.id = row.id;
      if (maxVersion > 0) data.currentVersionNumber = maxVersion;
      return data;
    },
