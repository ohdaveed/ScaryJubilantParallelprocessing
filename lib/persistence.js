...
const mapBuildQueueItemAsTodo = (row) => ({
  ...
  status: row.queue_status || "pending",
  errorMessage: row.error_message || null,
...
const mapPageConceptAsPlannedPage = (row) => ({
  id: row.id,
  name: row.canonical_title,
  pageType: row.content_type,
  userType: row.audience,
  parentId: row.parent_concept_id,
  builtPageId: row.canonical_artifact_id || null,
  createdAt: row.created_at
});
...
const createFileStore = async (filePath = DEFAULT_LOCAL_DB_PATH) => {
  ...
  const persist = () => {
    ...
    persistChain = persistChain
      .then(() => writeFile(resolvedPath, snapshot))
...
const createPostgresStore = (pool) => ({
  ...
  async createTodo(topic, userType, { plannedId: _plannedId } = {}) {
    const result = await pool.query(
      "INSERT INTO build_queue_items (topic, audience, queue_status, priority, requested_by) VALUES ($1, $2, 'pending', 50, 'ui') RETURNING *",
      [topic, userType]
    );
  ...
  async updateTodoQueue(id, { status, errorMessage, builtPageId, karlGrade }) {
    ...
    values.push(id);
    const result = await pool.query(
      `UPDATE build_queue_items SET ${columns.join(", ")} WHERE id = $${i} RETURNING *`,
      values
    );
    return result.rows[0] ? mapBuildQueueItemAsTodo(result.rows[0]) : null;
  },
  ...
  async listPlannedPages() {
    ...
    return result.rows.map(mapPageConceptAsPlannedPage);
  },
  async getPlannedPage(id) {
    const result = await pool.query("SELECT * FROM page_concepts WHERE id = $1", [id]);
    return result.rows[0] ? mapPageConceptAsPlannedPage(result.rows[0]) : null;
  },
  async createPlannedPage(name, pageType, userType, parentId) {
    const result = await pool.query(
      `INSERT INTO page_concepts
         (canonical_title, task_statement, content_type, audience, parent_concept_id, status, service_area, summary)
       VALUES ($1, $1, $2, $3, $4, 'proposed', 'hhvc', '') RETURNING *`,
      [name, pageType, userType, parentId ?? null]
    );
    return mapPageConceptAsPlannedPage(result.rows[0]);
  },
  async updatePlannedPage(id, patch) {
    const fields = [];
    const values = [];
    ...
    if (patch.name !== undefined) {
      fields.push(`canonical_title = $${index}`, `task_statement = $${index++}`);
    ...
    if (patch.userType !== undefined) {
      fields.push(`audience = $${index++}`);
      values.push(patch.userType);
    }
    if (patch.parentId !== undefined) {
      fields.push(`parent_concept_id = $${index++}`);
      values.push(patch.parentId);
    }
    if (patch.builtPageId !== undefined) {
      fields.push(`canonical_artifact_id = $${index++}`);
    ...
    values.push(id);
    const result = await pool.query(
      `UPDATE page_concepts SET ${fields.join(", ")}, updated_at = NOW() WHERE id = $${index} RETURNING *`,
      values
    );
    return result.rows[0] ? mapPageConceptAsPlannedPage(result.rows[0]) : null;
  },
