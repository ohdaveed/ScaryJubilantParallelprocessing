  ...
  if (pageArtifacts.length === 0 && pages.length > 0) {
    ...
    pageArtifacts = pages.map((row) => {
      ...
      return {
        ...
        ,
        status: data.qualityGate?.status === "review_required" ? "needs_review" : "draft",
        check_status: data.karlEvaluation ? "passed" : "not_checked",
        import_status: data.imported ? (data.reviewStatus === "approved" ? "approved" : data.reviewStatus === "rejected" ? "rejected" : "pending") : "none",
...
const initPostgres = async (pool) => {
  ...
  `);
  await pool.query(`ALTER TABLE page_concepts ADD COLUMN IF NOT EXISTS canonical_artifact_id TEXT`);
  ...
  `);
  await pool.query(`ALTER TABLE page_artifacts ADD COLUMN IF NOT EXISTS status TEXT`);
  await pool.query(`ALTER TABLE page_artifacts ADD COLUMN IF NOT EXISTS check_status TEXT`);
  await pool.query(`ALTER TABLE page_artifacts ADD COLUMN IF NOT EXISTS import_status TEXT`);
  await pool.query(`ALTER TABLE page_artifacts ADD COLUMN IF NOT EXISTS active_version_id INTEGER`);
  await pool.query(`ALTER TABLE page_artifacts ADD COLUMN IF NOT EXISTS verified_version_id INTEGER`);
  await pool.query(`ALTER TABLE page_artifacts ADD COLUMN IF NOT EXISTS is_canonical BOOLEAN DEFAULT false`);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS one_canonical_artifact_per_concept ON page_artifacts(concept_id) WHERE is_canonical = true`);
  ...
  const refCountResult = await pool.query(`SELECT COUNT(*) FROM reference_examples`);
  if (parseInt(refCountResult.rows[0].count) === 0) {
    for (const ex of DEFAULT_REFERENCE_EXAMPLES) {
      await pool.query(`
        INSERT INTO reference_examples (title, source_system, reference_type, notes, mapped_pattern, reference_map_id)
        VALUES ($1, $2, $3, $4, $5, $6)
      `, [ex.title, ex.source_system, ex.reference_type, ex.notes, ex.mapped_pattern, ex.reference_map_id]);
    }
  }
};
...
const createPostgresStore = (pool) => ({
  ...
  async createPageConcept(payload) {
    ...
    const allConceptsResult = await pool.query("SELECT * FROM page_concepts");
    const duplicateFlags = findConceptConflicts(
      allConceptsResult.rows.map(mapPageConcept),
      {
        canonicalTitle: row.canonical_title,
        taskStatement: row.task_statement,
        audience: row.audience,
    ...
    if (governance_flags.some((flag) => flag.severity === "error")) {
      throw new Error(governance_flags.map((flag) => flag.message).join(" "));
    }
    const result = await pool.query(
  ...
  async updatePageConcept(id, patch) {
    const existing = await pool.query("SELECT * FROM page_concepts WHERE id = $1", [id]);
  ...
  async createIANode(payload) {
    const now = new Date().toISOString();
    const countRes = await pool.query("SELECT COUNT(*) FROM ia_nodes");
  ...
  async updateBuildQueueItem(id, patch) {
    ...
    if (patch.queueStatus !== undefined) { fields.push(`queue_status = $${index++}`); values.push(patch.queueStatus); }
